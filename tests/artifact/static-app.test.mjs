...
test("the ethos line is verbatim in the head and the manifest", async () => {
  const ethos =
  ...
  assert.ok(description, "built app must include a description meta tag");
  assert.ok(description.includes(ethos), description);
  assert.match(description, /Works offline\./);

  const manifest = JSON.parse(
    await readFile(resolve(clientRoot, "manifest.webmanifest"), "utf8"),
  );
  assert.ok(manifest.description.includes(ethos), manifest.description);
});
