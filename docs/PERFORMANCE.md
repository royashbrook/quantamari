...
- The mash keeps only 12–24 recent rich toys; older pieces collapse into one
  ...
  size, its remaining toys temporarily use that same proxy draw.
- Projected pixel size—not a fixed distance—selects rich versus simplified
  pickup detail. The logarithmic free lens also changes semantic layer residency
  without changing saved journey progress.
